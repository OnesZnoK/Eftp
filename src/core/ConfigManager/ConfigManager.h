 ...
 *
 * 依赖模块：JsonHpp（JSON解析）
 * 被依赖方：SerApiModel、DownloadManager、NetworkMonitor
 */
...
public:
    ...
    // ── 环境检测（从 name_flag 文件读取）──
    std::string detectEnvironment() const;

    // ── DLL 路径（从 dll.ini 按环境读取）──
    std::string dllPathCore() const;      // 核心工具 DLL 路径
    std::string dllPathModules() const;   // 业务模块 DLL 路径
